			...
			for (i = response.length-1; i>=0 ; i--){
				if(response[i].isEmailRead == false){
					list.append('<li><span class="">'+
								...
								'<button type="button" id="read_'+ response[i].emailNotificationId+ '"onclick="showData('+response[i].emailNotificationId+')" class="btn btn-primary" style="left: 95%;position: relative;margin-top: -20px;">Read</button></span><time style="padding-right: 3%;">'+'</a></span></li>');
				}else{
					list.append('<li><span class="unread">'+
								...
								'<time>'+new Date(response[i].submittedDate)+'</time>'+
								'<span class="msg-body">'+response[i].emailSubject+'</span>'+
								'<button type="button" id="read_'+ response[i].emailNotificationId+ '"onclick="showData('+response[i].emailNotificationId+')" class="btn btn-primary" style="left: 95%;position: relative;margin-top: -20px;">Read</button></span></a></span></li>');
				}
