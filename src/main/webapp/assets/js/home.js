			...
			for (i = issueResponse.reportedIssueList.length-1; i>=0 ; i--){
				...
				if(issueResponse.reportedIssueList[i].updatedDate!=undefined&&issueResponse.reportedIssueList[i].updatedDate!=null){
					...
					list.append('<li><span>'+
								...
								'</span>'+
								'<div class="progress" style="height: 10px;"> <div class="progress-bar '+barStyle+' progress-bar-striped active" role="progressbar" aria-valuenow="'+issueResponse.reportedIssueList[i].percentageDone+'" aria-valuemin="0" aria-valuemax="100" style="width: '+issueResponse.reportedIssueList[i].percentageDone+'%"></div></div>'+
